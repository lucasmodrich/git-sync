...
func (c *GitlabClient) Sync(ctx context.Context, cfg config.Config) error {
	...
	gitSync.SyncWithConcurrency(ctx, cfg, projects, func(project *gl.Project) {
		...
		gitSync.CloneOrUpdateRepo(ctx, ns, name, repoAuthURL, cfg)
		...
		if cfg.IncludeWiki && project.WikiEnabled {
			...
			gitSync.SyncWiki(ctx, ns, name, wikiAuthURL, cfg)
		}
		if cfg.IncludeIssues && project.IssuesEnabled {
			...
			if err != nil {
				...
				gitSync.SyncIssues(ctx, project.Namespace.FullPath, project.Path, allIssues, cfg)
			}
		}
	})
